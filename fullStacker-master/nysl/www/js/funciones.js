...
function imprimirTabla(fechas) {
    ...
    for (var i = 0; i < fechas.length; i++) {
        ...
        tabla +=
            ...
            "<img class='redirection'  src='img/googleMaps.jpg' onclick=showPage('mapa') alt='HTML tutorial'>" +
            "                       " +

            "<a class='redirection' href='https://www.google.com/calendar'>" +
            "<img src='img/calendar.jpg' alt='HTML tutorial'>" +
            "</a>" +

            "<img class='redirection' src='img/iconoMen.jpg'" +
...
function enviarMensaje(tipoDeMensaje) {
    
    var mensajeAEnviar = document.getElementById(tipoDeMensaje).value;   
    
    var displayLaterMassage = document.getElementById("sendLater").checked;
    
     connectedRef.on("value", function (snap) {

        if (snap.val() === true && !esMesajeVacio(mensajeAEnviar)) {
            
            agregarMensajeABase(mensajeAEnviar);

            console.log("se ha enviado un mensaje");
            
            document.getElementById(tipoDeMensaje).value = " ";
            
        } else if (displayLaterMassage == true) {
            
            seEnvioMensajeOffline = true;
            
            var nombreMensaje = "mensaje" + getId();

            localStorage.setItem(nombreMensaje, mensajeAEnviar);  
...
function enviarMensajesAlmacenados() {
    ...
    for (var i = cantElemEnviados; i > 0; i--){
        
        var nombreImagen = "imagen" + i;
        var nombreMensaje = "mensaje" + i;
        
        if (localStorage(nombreImagen) != null) {
            
            var uriDeImagenAEnviar = localStorage.getItem(nombreImagen);
            var fileImage = prepareFile(uriDeImagenAEnviar,nombreImagen);
            agregarImagenABase(fileImage);
            localStorage.removeItem(nombreImagen);
            cantElemEnviados= cantElemEnviados - 1;
        
        }
        
        if (localStorage(nombreMensaje) != null) {
            
            var mensajeAEnviar = localStorage.getItem(idMensaje);
            agregarMensajeABase(mensajeAEnviar);
            localStorage.removeItem(nombreMensaje);
            cantElemEnviados= cantElemEnviados - 1;
...
function agregarMensajeABase(stringMensaje){
...
function offLine() {
    ...
    var frecuenciaEnSegundos = 60000 * valorFrecuencia;
    
        if (displayLaterMassage == true) {
            console.log("comienza time out");
           setTimeout(enviarMensajesAlmacenados, frecuenciaEnSegundos);
